...
export const AIInterface: React.FC<AIInterfaceProps> = ({ isOpen, onToggle }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.error(err);
      setChatMessages(prev => [...prev, { role: 'ai', content: 'Sorry, could not reach the AI server.' }]);
    } finally {
      setIsTyping(false);
    }
  };
      ...
      <div className="bg-background/95 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden animate-scale-in">
        ...
        <div className="bg-gradient-primary p-6 border-b border-white/20">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={onToggle} variant="ghost" size="icon" className="text-white hover:bg-white/20 rounded-xl">×</Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 max-h-[calc(90vh-120px)] overflow-y-auto">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <Card className="glass-card border-0">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <MessageCircle className="w-5 h-5 text-wellness-calm" /> Chat with AI
                </CardTitle>
              </CardHeader>
                ...
                <div className="h-64 overflow-y-auto space-y-3 p-4 bg-muted/30 rounded-xl">
                  {chatMessages.map((msg, index) => (
                    <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] p-3 rounded-2xl ${msg.role === 'user' ? 'bg-gradient-primary text-white' : 'bg-card border border-border'}`}>
            ...
            <Card className="glass-card border-0">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-wellness-serene" /> AI Insights
                </CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <div className="space-y-4">
            <Card className="glass-card border-0">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Activity className="w-5 h-5 text-wellness-peaceful" /> Wellness Trends
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-48">
            ...
            <Card className="glass-card border-0">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Zap className="w-5 h-5 text-wellness-warm" /> Quick Actions
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <Button variant="outline" className="w-full justify-start">
                  <Bot className="w-4 h-4 mr-2" /> Mood Check-in
                </Button>
                <Button variant="outline" className="w-full justify-start">
                  <Brain className="w-4 h-4 mr-2" /> Stress Assessment
                </Button>
                <Button variant="outline" className="w-full justify-start">
                  <Activity className="w-4 h-4 mr-2" /> Wellness Plan
                </Button>
              </CardContent>
            </Card>
